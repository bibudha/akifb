...
var app = {
    ...
    getReservationHTML: function () {
        ...
        //  data += '</ul>';
        data += '<div data-role="collapsible" data-collapsed="false" data-content-theme="d"><h3>Upcoming reservations</h3><div id="divUpcomingList"></div>';
        data += '</div>';
    ...
    getScheduleReservation: function () {
        ...
        if (!isNullOrEmpty(timeJSON) && i == 2) {
            ...
            selectedTime += '<li id="liTime" data-theme="c" date="' + obj.reservationDate + '">' + obj.reservationTime + '</li>';
        }
            ...
            if ($.isEmptyObject(html)) {
                ...
                if (html.length > 0) {
                    ...
                    if (openStatus == "1") {
                        ...
                        $.each(html, function (i, item) {
                            if (isNullOrEmpty(item.openStatus)) {
                                data += '<li style="position:relative" reservationId="' + item.reservationId + '" duration="' + item.duration + '" currency="' + item.currency + '"  reservationId="' + item.reservationId + '" reservationId itemid="' + item.ItemId + '" serviceName="' + item.serviceName + '"  reservationFee="' + item.reservationFee + '">';
                                data += '<div><h3>' + item.serviceName + '</h3></div><div><input style="position:absolute;right:10px; top:10px;" onclick="app.res_addService(this)" type="button" data-role="button" value="book it" /></div>';
    ...
    res_addTime: function (btn) {
        ...
        var reservationDate = $('div[data-role=header] h1').html();
        var reservationTime = $(btn).html();
        var timeJSON = '{"reservationTime":"' + reservationTime + '","reservationDate":"' + reservationDate + '"}';
        ...
        if (registerid != null) {
            ...
            doAjaxCall(url, null, false, function (html) {
                ...
                if ($.isEmptyObject(html)) {
                    ...
                    if (html.length > 0) {
                        data += '<div data-role="collapsible" data-collapsed="false" data-content-theme="d"><h3>Reservations history</h3><div id="divUpcomingList"></div>';

                        data += '<div data-role="collapsible-set" data-theme="c" data-content-theme="d">';
                        $.each(html, function (i, item) {
                            var date = new Date(item.reservationDate * 1000);
                            var _date = date.getDate();
                            var month = date.getMonth();
                            var year = date.getFullYear();
                            data += '<div data-role="collapsible" data-collapsed="false"> <h3>' + item.serviceName + '</h3>';
                            data += '<p>Service Name: ' + item.serviceName + '</p>';
                            data += '<p>Price: ' + item.currency + ' ' + item.paymentAmt + '</p>';
                            data += '<p>Reservation date: ' + item.time + '</p>';
                            data += '<p>Reservation time: ' + _date + '/' + month + '/' + year + '</p>';
                            data += '</div>';

                        });
                        data += '</div></div>';
                    } else {
                        data += 'No upcoming reservation found';
                    }
                }
                $('#main-content').html(data);
                try {
    ...
    res_scheduledinfo: function () {
        ...
        if (registerid != null) {
            ...
            var url = baseUrl + 'web/web/reservationhistoryinfo/' + registerid + '/1';
    ...
    makeReservation: function () {
        ...
        var reservationfee = $('#liService').attr('reservationfee');
        var reservationDate = $('#liTime').attr('date');
        var cart = '{ "reservationId":  "' + reservationId + '", "locationId": "' + locationId + '", "serviceId": "' + serviceId + '", "timestr":  "' + timeString + '", "featureRelId":  "' + featureRelId + '", "currency":"' + currency + '", "serviceName":"' + serviceName + '", "price":"' + reservationfee + '","reservationDate":"' + reservationDate + '","address":"' + address + '" }';
        setCookie("cart", cart);
        if (app.getReservationUser() != null) {
            window.location.href = paymentPageUrl;
    ...
    loadPaymentInfo: function () {
        ...
        if (app.getReservationUser() != null && cart != null) {
            ...
            data += '<input type="hidden" name="reservationDate" value="' + reservationDate + '" />';
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data == "true" || data == true) {
                        ...
                        window.location.href = "app_reservation_success.html";
                    } else {
                        console.log(data);
                        // redirect to error page
                        // window.location.href = "app_reservation_error.html";
                    }
            ...
            if ($.isEmptyObject(html)) {
                ...
                $.each(html, function (i, item) {
                    if (i == 0) {
                        ...
                        data += '<div id="fb-root"></div><div class="tabcontainer"><div data-role="navbar">';
